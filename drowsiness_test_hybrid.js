...
// Backend API URL - use same host as web server
...
async function init() {
    try {
        ...
        console.log('Click "Start Visualization" to enable webcam');
...
async function setupWebcam() {
    try {
        cameraSource = cameraSourceSelect.value;
        ...
        if (cameraSource === 'laptop') {
            ...
            console.log('✅ Laptop webcam ready');
        } else {
            // Use Raspberry Pi camera stream
            webcam.src = CAMERA_STREAM_URL;
        ...
        if (cameraSource === 'laptop') {
            ...
            alert('Cannot connect to Pi camera stream. Make sure camera_stream.py is running.');
        }
    }
}
...
async function toggleVisualization() {
    ...
    if (isVisualizationActive) {
        // Setup webcam if not already setup
        if (!webcamStream) {
            try {
                await setupWebcam();
            } catch (error) {
...
function stopVisualization() {
    ...
    // Stop webcam stream if using laptop camera
    if (cameraSource === 'laptop' && webcamStream) {
